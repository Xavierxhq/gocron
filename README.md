...
[![CI State](https://github.com/Xavierxhq/gocron/actions/workflows/go_test.yml/badge.svg?branch=v2&event=push)](https://github.com/Xavierxhq/gocron/actions)
![Go Report Card](https://goreportcard.com/badge/github.com/Xavierxhq/gocron) [![Go Doc](https://godoc.org/github.com/Xavierxhq/gocron/v2?status.svg)](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2)
...
```
go get github.com/Xavierxhq/gocron/v2
```
...
- [Go doc examples](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#pkg-examples)
...
- [**Duration**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#DurationJob):
Jobs can be run at a fixed `time.Duration`.
- [**Random duration**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#DurationRandomJob):
Jobs can be run at a random `time.Duration` between a min and max.
- [**Cron**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#CronJob):
Jobs can be run using a crontab.
- [**Daily**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#DailyJob):
Jobs can be run every x days at specific times.
- [**Weekly**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WeeklyJob):
Jobs can be run every x weeks on specific days of the week and at specific times.
- [**Monthly**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#MonthlyJob):
Jobs can be run every x months on specific days of the month and at specific times.
- [**One time**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#OneTimeJob):
...
- [**Per job limiting with singleton mode**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WithSingletonMode):
...
- [**Per scheduler limiting with limit mode**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WithLimitConcurrentJobs):
...
- [**Elector**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WithDistributedElector):
...
other instances checking to see if a new leader needs to be elected.
  - Implementations: [go-co-op electors](https://github.com/Xavierxhq?q=-elector&type=all&language=&sort=)
    (don't see what you need? request on slack to get a repo created to contribute it!)
- [**Locker**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WithDistributedLocker):
...
Locker can be at job or scheduler, if it is defined both at job and scheduler then locker of job will take precedence.
  - Implementations: [go-co-op lockers](https://github.com/Xavierxhq?q=-lock&type=all&language=&sort=)
    (don't see what you need? request on slack to get a repo created to contribute it!)

### Events
Job events can trigger actions.
- [**Listeners**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WithEventListeners):
Can be added to a job, with [event listeners](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#EventListener),
or all jobs across the
[scheduler](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WithGlobalJobOptions)
to listen for job events and trigger actions.

### Options
Many job and scheduler options are available.
- [**Job options**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#JobOption):
Job options can be set when creating a job using `NewJob`.
- [**Global job options**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WithGlobalJobOptions):
Global job options can be set when creating a scheduler using `NewScheduler`
and the `WithGlobalJobOptions` option.
- [**Scheduler options**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#SchedulerOption):
Scheduler options can be set when creating a scheduler using `NewScheduler`.

### Logging
Logs can be enabled.
- [Logger](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#Logger):
...
- [**Monitor**](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#Monitor):
A monitor can be used to collect metrics for each job from a scheduler.
  - Implementations: [go-co-op monitors](https://github.com/Xavierxhq?q=-monitor&type=all&language=&sort=)
    (don't see what you need? request on slack to get a repo created to contribute it!)
...
to [WithClock](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#WithClock) -
see the [example on WithClock](https://pkg.go.dev/github.com/Xavierxhq/gocron/v2#example-WithClock).
